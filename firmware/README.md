...
On Windows, double-click `nexus-run-firmware.cmd` to upload through built-in USB-JTAG and open Serial Monitor. Every monitor session is saved automatically under the repository's `logs/` directory. The current backend does not own the COM port, so close Serial Monitor before another program tries to open the same device.
...
The one-click `nexus-monitor-firmware.cmd` and `nexus-run-firmware.cmd` launchers add PlatformIO's `log2file` filter automatically. Each session is stored as `logs/device-monitor-YYMMDD-HHMMSS.log` relative to the repository root. Keep the monitor open for the full baseline interval, then press `Ctrl+C` so the file is flushed and closed cleanly.
