...
| Backend | FastAPI, serial auto-detection, telemetry validation, WebSocket fan-out and MVP diagnostics | Add Nemotron adapter and orchestration |
...
The backend owns the serial port, rejects malformed/`nan` packets, and streams complete live snapshots over `/api/v1/live/ws`. Nemotron and device orchestration remain later backlog items.
